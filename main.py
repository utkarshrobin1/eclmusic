...
# ── Debug: log every incoming message (remove after testing) ──────────────────
@bot.on_message(filters.all, group=-1)
async def debug_all(client, message: Message):
    print(f"📨 Message received | chat={message.chat.id} | text={message.text!r}")
...
async def main():
    ...
    try:
        await bot.send_message(
            OWNER_ID,
            "✅ **Elite Musico is online!**\nSend /ping to test."
        )
    except Exception as e:
        print(f"⚠️ Could not notify owner: {e}")

    print("✅ Elite Musico started! Waiting for messages...")
    await asyncio.Event().wait()  # blocks forever, keeps bot alive
